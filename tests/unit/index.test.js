...
test("Fetch Unlocked Cards", () => {
    ...
    const card_data = fetch_unlocked_cards(card_obj)
    expect(card_data.length).toBe(1);
    expect(card_data[0].name).toBe(card_obj.name);
    expect(card_data[0].bio).toBe(card_obj.bio);
    expect(card_data[0].course).toBe(card_obj.course);
    expect(card_data[0].rarity).toBe(card_obj.rarity);

    //When data is stored, it should return the data stored
    save_to_local([card_2], "card_data");
    const card_data_2 = fetch_unlocked_cards(card_obj);
    expect(card_data_2.length).toBe(1);
    expect(card_data_2[0].name).toBe(card_2.name);
    expect(card_data_2[0].bio).toBe(card_2.bio);
    expect(card_data_2[0].course).toBe(card_2.course);
    expect(card_data_2[0].rarity).toBe(card_2.rarity);
